...
import csv, io
from project_finance import base_case, calculate_model, ProjectModelInput

app = FastAPI(title="Generic Project Finance API", version="0.1.0")
SCENARIOS: dict[str, ProjectModelInput] = {"base": base_case()}

class ScenarioPayload(BaseModel):
    name: str
    inputs: ProjectModelInput
...
def calculate_custom(inputs: ProjectModelInput): return calculate_model(inputs)
...
def sensitivity(inputs: ProjectModelInput, variable: str='revenue.colocation_price_per_kw_month', shocks: list[float]=[-0.1,0,0.1]):
