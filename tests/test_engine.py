...
from project_finance import base_case, calculate_model
from project_finance.schemas import FinancingTranche

def test_base_case_calculates_and_converges():
    r=calculate_model(base_case())
    assert r['summary']['total_capacity_mw'] == 12
    assert r['summary']['total_project_cost'] > 150_000_000
...
def test_multiple_tranches_and_shareholder_loan():
    inp=base_case(); inp.financing.append(FinancingTranche(name='Shareholder Loan', instrument_type='shareholder_loan', commitment=10_000_000, gearing_pct=0.05, interest_rate=0.12, amortization_start=date(2030,1,31), final_maturity=date(2040,12,31), repayment_method='annuity'))
    r=calculate_model(inp)
    assert r['summary']['debt_required'] > 0

def test_disabled_phase_excluded():
    inp=base_case(); inp.phases[1].enabled=False
    assert calculate_model(inp)['summary']['total_capacity_mw'] == 12
